...
def cache_base(path, without_slash=True):
    if without_slash:
        ...
        path = trim_base(path).replace(' ', '_').replace('(', '').replace('&', '').replace(',', '').replace(')', '').replace('#', '').replace('[', '').replace(']', '').replace('"', '').replace("'", '').replace('_-_', '-').lower()
    while path.find("--") != -1:
        path = path.replace("--", "-")
...
def image_cache(path, size, square=False, without_slash=True):
    if square:
        suffix = str(size) + "s"
    else:
        suffix = str(size)
    return cache_base(path, without_slash) + "_" + suffix + ".jpg"
def file_mtime(path):
